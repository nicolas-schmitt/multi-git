...
module.exports = class MultiGit {
            ...
            return repository.git
                ...
                .then((pendingChanges) => {
                    ...
                    if (repository.currentBranch && repository.currentUpstream) {
                        return Promise.all([
                            repository.git.getBranchCommit(repository.currentBranch.shorthand),
                            repository.git.getBranchCommit(repository.currentUpstream.shorthand)
                        ]);
                    }
                })
                .then((commits) => {
    ...
    fetch(repositories) {
        let proms = _.map(
            ...
            (repository) => {
                return repository.git
                    .fetch(repository.currentUpstream.remote)
                    .then(() => {
                        return repository;
                    })
                    .catch((err) => {
                ...
                let proms = _(result).filter((repository) => {
                    ...
                    .map((repository) => {
                        if (rebase) {
                            return repository.git
                                .rebaseBranches(
                                    repository.currentBranch.shorthand,
                                    repository.currentUpstream.shorthand
                                )
                                .catch((err) => {
                                    console.error('[Multi-Git] Error rebasing', repository.name, err);
                                });
                        } else {
                            return repository.git
